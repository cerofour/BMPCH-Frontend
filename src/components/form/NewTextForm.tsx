...
export function NewTextForm({ setShow, isEditMode, id }: any) {
	...
	const mutation = useMutation({
		...
		onSuccess: (data) => {
			...
			context?.setToastData(
				isEditMode ? "Texto actualizado exitosamente" : "Texto creado exitosamente.",
				"success"
			);
			setShow(false);
		},
		onError: () => {
			context?.toggleToast();
			context?.setToastData(
				isEditMode ? "No se ha podido actualizar el texto" : "No se ha podido crear el texto.",
				"danger"
			);
		},
	});
	...
	const handleSubmit = (e: FormEvent) => {
		...
		if (isEditMode) mutation.mutate({ id: id, data: formData });
		else mutation.mutate({ data: formData });
	};
